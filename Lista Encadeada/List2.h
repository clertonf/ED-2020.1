...
class List {
   ...
   public:
      ...
      void insertAfter(int x, int k); /* Insere um novo nó com valor x após o k-ésimo nó da lista. Deve-se 
                                 ter 0 ≤ k ≤ size() para que a inserção seja realizada; caso contrário, 
                                 não sera realizada. */
      List * copy();/* Retorna um ponteiro para uma cópia desta lista. */
      void copyArray( int v[], int n );/* Copia os elementos do array v para a lista. O array tem n elementos. 
                                          Todos os elementos anteriores da lista são apagados. */
      bool equal(List *lst);/* Determina se a lista lst, passada por parâmetro, é igual à lista em questão. 
      ...
      void concatenate( List *lst );/* Concatena a lista atual com a lista lst passada como parâmetro para a função. 
                                    Após essa operação, lst será uma lista vazia, ou seja, o único nó de lst sera 
                                    o nó cabeça. */
      void reverse();/* Inverte a ordem dos nós (o primeiro nó passa a ser o último, o segundo passa a ser o penúltimo, etc.) 
